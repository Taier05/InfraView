...
func TestLoadDefaults(t *testing.T) {
	...
	if cfg.SMARTCollectionInterval != 60*time.Second {
		t.Fatalf("SMART collection interval = %s", cfg.SMARTCollectionInterval)
	}
...
func TestLoadAllValues(t *testing.T) {
	cfg, err := Load(mapEnv(map[string]string{
		...
		"INFRAVIEW_SMART_COLLECTION_INTERVAL":           "75s",
	...
	if cfg.SMARTCollectionInterval != 75*time.Second {
		t.Fatalf("SMART collection interval = %s", cfg.SMARTCollectionInterval)
	}
...
func TestLoadRejectsInvalidValues(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid SMART collection interval", overrides: map[string]string{"INFRAVIEW_SMART_COLLECTION_INTERVAL": "later"}, wantError: `INFRAVIEW_SMART_COLLECTION_INTERVAL 必须是有效时长，当前值为 "later"`},
		{name: "zero SMART collection interval", overrides: map[string]string{"INFRAVIEW_SMART_COLLECTION_INTERVAL": "0s"}, wantError: `INFRAVIEW_SMART_COLLECTION_INTERVAL 必须大于 0，当前值为 "0s"`},
		{name: "negative SMART collection interval", overrides: map[string]string{"INFRAVIEW_SMART_COLLECTION_INTERVAL": "-1s"}, wantError: `INFRAVIEW_SMART_COLLECTION_INTERVAL 必须大于 0，当前值为 "-1s"`},
		{name: "SMART collection interval below one second", overrides: map[string]string{"INFRAVIEW_SMART_COLLECTION_INTERVAL": "500ms"}, wantError: `INFRAVIEW_SMART_COLLECTION_INTERVAL 必须是不小于 1s 的整秒时长，当前值为 "500ms"`},
		{name: "SMART collection interval with fractional second", overrides: map[string]string{"INFRAVIEW_SMART_COLLECTION_INTERVAL": "1500ms"}, wantError: `INFRAVIEW_SMART_COLLECTION_INTERVAL 必须是不小于 1s 的整秒时长，当前值为 "1500ms"`},

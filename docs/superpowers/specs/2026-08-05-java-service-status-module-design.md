...
- `internal/javaapp` 与 `internal/service` 中的进程数、内存字节数和运行时间继续使用 nullable `int64`，服务端排序也直接比较 `int64`。HTTP 的 `process_count`、`memory_bytes`、`uptime_seconds` 为避免 JavaScript `number` 超过 `2^53-1` 后丢失精度，固定传输为规范十进制字符串或 `null`：只接受 `0` 或非零数字开头的十进制数字串，不允许正负号、科学计数法或非规范前导零，且不得超过 `9223372036854775807`。前端必须用 `BigInt` 或等价无损整数逻辑校验和格式化，禁止先转为 `number`。
...
服务列表响应中的精确整数 transport 固定为：`process_count: string|null`、`memory_bytes: string|null`、`uptime_seconds: string|null`。例如超过 JavaScript 安全整数边界的 `9007199254740993` 必须原样编码为 JSON 字符串 `"9007199254740993"`；`MaxInt64` 必须原样编码为 `"9223372036854775807"`，不能编码为 JSON number、科学计数或舍入值。
...
- HTTP：锁定两个认证 GET、显式 View、非 null 数组、参数白名单、重复/空/非法参数 400、405、503、stale、敏感字段排除，以及 `2^53+1` 与 `MaxInt64` 的规范十进制字符串编码。
- 前端：锁定第七张卡、四类摘要、13 个精确表头、每行 13 个单值单行单元格、共享控制栏、URL 恢复、BigInt 无损格式化、精确整数 transport 校验、百分比严格 `0..100`、空/错/stale/刷新状态和无破坏性控件。

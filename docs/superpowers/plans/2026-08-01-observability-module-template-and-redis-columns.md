...
十一列/共享模板及后续运行时间纠错均已获授权并原位重建现有测试 8080；提交、推送未获授权，不执行。后续已授权修复验证通过后自动重建同一测试 8080。
...
把“累计整小时”更正为“与主机/MySQL 相同的天/小时格式”，记录 RED→GREEN 与部署结果；运行 `git diff --check`。按用户长期授权直接原位重建同一测试 8080；不提交、不推送。

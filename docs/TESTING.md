...
- 领域与服务测试覆盖稳定实例标识、Provider 契约、可用性与复制状态聚合、缺失值语义、缓存与 stale 回退，以及搜索、筛选、排序和分页。
- Nightingale 契约测试全部使用脱敏 `httptest` 夹具，覆盖固定 PromQL、完整实例身份、单次 instant batch、实例与复制通道合并、冲突/非法值降级，以及错误不泄漏 Token、响应正文、标签或查询。
- HTTP 测试覆盖 `/api/v1/mysql/overview` 和 `/api/v1/mysql/instances` 的认证、完整 schema、空数组、固定查询参数、只允许 GET、写方法 405、安全 503 和未装配服务时的失败语义。
- Vitest 覆盖总览 MySQL 卡的独立加载、错误、stale 与刷新状态，以及实例页 11 列、中文状态、空指标、GET 参数白名单、URL 状态、分页和后台刷新错误。

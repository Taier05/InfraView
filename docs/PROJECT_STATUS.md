...
### 2026-07-30 样本推进新鲜度修复（已部署测试 8080）
...
当前工作区已按 TDD 改为并发安全的进程内样本推进状态：首次观察建立本地基线，原始时间变化时重置本地推进时刻，同一时间连续 30 秒未变化进入警告、75 秒未变化进入严重，恢复推进立即正常；时间回退重新建立基线。Provider 的 `tlast_over_time`、MySQL 16 条固定单 batch、24 小时身份与 API 契约不变。主机“采集延迟/采集失联”颜色同步改为 warning 黄色/critical 红色，与 MySQL 一致。

无缓存完整镜像已通过前端全测、TypeScript、生产构建、Go 全仓普通/race 测试和编译；原有仅连接测试 Nightingale 的 8080 已原位重建并健康。脱敏登录态现场观察跨越多个缓存刷新和至少两个采集周期，Linux/MySQL 采集等级持续正常且响应非 stale。冻结 2/5 周期升级及恢复推进由自动化测试覆盖，本轮未再次操作测试采集器。未提交或推送。

### 2026-07-30 原始样本绝对年龄修复（已部署测试 8080，判定逻辑已被替代）

测试采集器停止后已确认旧实现会被 Nightingale Target 更新时间和即时查询外层求值时间掩盖：Linux 主机保持正常，MySQL 直到上游即时回看窗口结束才告警。当前工作区已按 TDD 修复：Linux 固定 batch 增加 `tlast_over_time(system_uptime[24h])`，MySQL 第 16 条固定查询改为 `tlast_over_time(mysql_up[24h])`；两者都只使用查询值中的原始最后样本 Unix 时间计算采集新鲜度。

Target 状态仍用于资产在线/离线判定，但不再参与采集新鲜度。默认 `15s` 周期与 2/5 周期阈值不变，即 30 秒警告、75 秒严重；MySQL 仍保持 16 条固定查询、一次 batch、24 小时近期身份和无 N+1。无缓存生产镜像构建已通过前端 71/71、TypeScript、生产构建、Go 全仓普通/race 测试和编译；既有仅连接测试 Nightingale 的原 8080 已原位重建并健康。脱敏登录态现场验收确认停采测试对象在主机、MySQL 和两张总览卡均为严重，响应均非 stale。尚未提交或推送。

### 2026-07-29 采集新鲜度与 MySQL TPS（已部署，随后发现时间口径缺陷）

当前 `main` 工作区已按授权实现但尚未提交：新增默认 `15s` 的预期采集周期，Linux 主机与 MySQL 实例达到 2 个周期进入警告、达到 5 个周期进入严重；页面分别显示“采集延迟”和“采集失联”。Nightingale 不再为全空当前指标伪造当前时间。
...
2026-07-30 已获用户授权，使用现有受限配置原位强制重建既有 `infraview` 服务；仍只有原 8080，仍为测试 Nightingale 模式。容器健康，无正文健康检查成功，受保护 API 未认证仍返回拒绝访问。尚未执行登录态 Chromium 验收，未连接生产、未提交或推送。
...
- 部署：当前 8080 InfraView 容器已使用更新后的私密环境文件重建，容器健康，Nightingale 数据源健康且非 stale。
...
- 当前 v8.4.1 验收：认证、Target、数据源、即时批量和区间批量只读契约通过；应用端数据源、总览、主机、单机详情和 1 小时指标范围 smoke 均通过且非 stale。
...
当前没有已知 Nightingale v8.4.1 API 契约或部署验收阻塞。生产试运行中，除 IO 忙碌度显示“暂无数据”外，已有板块指标正常；当前 InfraView 固定读取 `diskio_io_util`，而该生产环境目前可查询 `diskio_io_time` 派生利用率。用户决定先升级生产 Categraf，暂不修改 InfraView；因此这是一项等待采集端升级后复验的兼容观察，不是已经确认的 InfraView 缺陷。
...
- Compose 服务：`infraview`，已使用更新后的私密环境文件重建，InfraView 进程健康。
- 运行验证：数据源为真实 `nightingale` 且健康、非 stale；总览、主机、单机详情和 1 小时指标范围只读 smoke 通过。
...
1. 等待生产 Categraf 升级完成，至少经过两个采集周期后，只读验证 `diskio_io_util` 是否出现以及 InfraView 自动刷新后是否恢复展示。
2. 若升级后仍缺失，只收集脱敏的指标名、标签形状、单位和查询结果状态；只有用户明确授权后，才按 TDD 调整兼容逻辑。
3. 后续落实专用最小只读 Token，并按证据补充磁盘容量和磁盘读写历史指标。

...
### 2026-08-08 列表单页最多 500 条、共享运行时长与硬盘错误摘要（已部署测试 8080，未合并/未推送）

本节仅说明隔离工作树 `/root/github/InfraView/.worktrees/list-duration-disk-errors` 的 `feature/list-duration-disk-errors`。七个只读列表（主机、硬盘、MySQL、Redis、Elasticsearch、RabbitMQ、Java）现共同接受 `20|50|100|500` 的 `page_size`；页面显示普通“500 条”选项，切换后回到第 1 页并通过 URL 恢复。服务端仍对完整筛选结果排序后分页，超过 500 条仍继续分页，不做客户端拼接、静默截断、额外请求或虚拟滚动；API 响应结构、15 秒刷新和只读边界不变。

共享运行时长已交付：六个运行时间列和硬盘通电时间均使用共享 formatter，固定按 365 天计算年并只显示非零年、天、小时、分钟。硬盘仅把有限非负 `power_on_hours` 换算为秒，合法小数小时保留真实分钟，缺失或非法值安全显示“暂无数据”；页面正文与原生 `title` 相同，原始服务端排序仍为 `power_on_hours`，点击后仍回到第 1 页。硬盘错误摘要与固定 `smart_device_command_timeout` 已完成：仅显示已知非零项、最多两项后明确省略、全零/部分缺失/全缺失语义准确；命令超时仅参与摘要与排序，不触发硬盘状态、`status_source` 或总览告警。

分页、时长与硬盘错误摘要交付仍仅保留在该 feature 分支本地，已原位重建现有测试 8080；未合并到 `main`、未推送。镜像构建门禁与部署健康/安全核验通过，未创建额外端口、未启动浏览器、未输出任何现场数据。实际 main/远端状态必须在继续前重新核对。
...
容器前端门禁已新鲜通过：Vitest 16 个文件、275 项测试，随后 typecheck、生产构建及 Playwright 27 项静态清单均以退出码 0 完成。静态扫描确认 Java 页面只有固定 `GET` 请求，未发现写方法、命令执行、重启、删除或任意 PromQL；暂存前 whitespace 检查无输出。未启动服务或浏览器、未发布端口、未访问上游、未读取私密环境，且未输出真实标识或现场数据。

### 2026-08-07 数据时间语义与 Elasticsearch 瞬时过期修复（已合并、推送并部署）

当时分支 `feature/refresh-data-time-es-stale` 已按 TDD 完成两项核心修复：所有 Service 的 `meta.collected_at` 改为响应内最新有效 Nightingale 样本时间，空响应不伪造时间；正常页面删除无效手动刷新控件，总览与列表分别展示模块自己的格式化最新数据时间，自动轮询与错误态重试保留。前端定向回归已通过七个列表页 150/150、共享组件与总览 79/79。

Elasticsearch Provider 仍使用固定一次 26 查询 batch；第 25/26 组 inventory 继续硬失败，第 1–24 组可选指标允许单组 `null` 并按字段缺失展示，避免可选序列瞬时空结果把整个模块错误标记为 stale。最终验证已通过前端 16 文件/273 项、typecheck/build、Playwright 5 文件/27 项静态发现、Go gofmt/vet/全仓普通/race/编译及无缓存生产镜像。经用户授权，现有测试 8080 已从当前实现原位重建；健康、唯一服务与 8080、安全基线和未认证 API 拒绝均通过，未创建额外端口。用户已完成视觉验收并确认结果良好；功能提交 `ad6f01b` 已快进合并到 `main` 并推送至 `origin/main`。未读取私密环境内容，也未连接生产上游。
...
最终修复后的容器化静态验证包括前端 16 文件/273 项、typecheck、production build、Java Playwright 静态发现 1 文件/6 项、Go gofmt/vet/普通/race/编译和安全/whitespace/diff 扫描。功能提交 `5ed392a` 已快进合并到 `main` 并推送到 `origin/main`。经单独授权，现有 8080 已用该提交原位重建；镜像内前端 273 项、typecheck/build、Go 普通/race/编译通过，服务 healthy，仅发布原 8080，并保持非 root、只读根文件系统、cap drop `ALL` 与禁止提权。动态 Playwright/浏览器未执行；未运行会创建额外端口的 `scripts/e2e.sh`，未读取私密环境内容，也未连接或探测生产 Nightingale/Java 服务。npm 的锁文件审计提示和 Vite 第三方 `"use client"` 提示为预存依赖警告，未修改依赖或执行 audit fix。
...
### 历史记录：2026-07-27 至 2026-07-28 状态、刷新与数据连接优化
...
总览页当时升级为可扩展的“板块告警摘要卡”模式：当时只保留一张普通单列宽度的可点击 `Linux 主机` 卡，优先显示整体正常/警告/严重状态、异常主机数、严重/警告主机数，以及 CPU、内存、IO、网络各自异常数量和等级构成；底部保留在线、离线和未知数量，点击整张卡进入主机列表。离线主机计严重、未知主机计警告，指标沿用主机列表阈值，同一主机只按最高等级计一次。总览不再展示 CPU、内存平均值卡、资源趋势或时间范围控件；后续服务、网络、存储等板块按相同尺寸逐张加入。

总览告警汇总由后端复用已经获取的全部主机当前指标在内存中计算，不增加数据源请求、不存储告警事件。总览和主机页现在共用刷新控制组件，统一展示刷新按钮、上次成功刷新时间、后端运行时下发的自动刷新周期和刷新中状态。

该历史阶段的主机表格为主机名、IP 地址、CPU 核数、内存容量、CPU 使用率、内存使用率、负载、IO 忙碌度、网络出/入、运行时间、状态共 11 列，并使用 `⇅/↑/↓` 排序箭头；这只是当时实现，不代表本分支当前 12 列、网络收发拆列且无可见箭头的契约。其余当时行为包括 20/50/100 分页、单行省略、缺失值末置、阈值着色，以及已删除的主机详情与图表前端。

- 当时开发分支：`main`
- 当时功能交付基线：`6300413`，已推送到 `origin/main`
- 主工作树：`/root/github/InfraView`
- 当时预览 worktree：无；原 v8.4.1 兼容 worktree 已清理
...
当时没有已知 Nightingale v8.4.1 主机硬盘 SMART 契约阻塞；当时测试 8080 的 API、Chromium 和样本推进验收已经通过。历史生产试运行中，除 IO 忙碌度显示“暂无数据”外，已有板块指标正常；当时 InfraView 固定读取 `diskio_io_util`，而该环境可查询 `diskio_io_time` 派生利用率。用户当时决定先升级生产 Categraf，暂不修改 InfraView；这是一项历史兼容观察，不代表当前工作树阻塞。
...
1. 当时硬盘功能已提交并推送；该记录不代表本分支当前 push、merge 或部署状态。
...
历史授权记录：当时曾约定特定 Redis 修复可原位重建既有 8080；该旧授权不适用于当前 `feature/list-table-consistency` 工作树，也不构成本次部署、重启、push 或 merge 授权。

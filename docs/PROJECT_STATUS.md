...
- 真实浏览器：官方 Playwright Chromium 隔离环境 3/3 通过；1440×900 总览无横向或纵向溢出，告警摘要卡完整显示。主机页选择每页 50 条后 URL 保持 `page_size=50`，32 台 Mock 主机可在同页渲染且无横向溢出。
- 部署：当前 8080 预览已重建为告警摘要与可选每页数量版本，容器健康，登录后 smoke PASS。

...
Nightingale 测试环境已确认版本、拓扑、API 基础地址、Token 请求头、3 台 Target 和实际指标映射。当前阻塞收敛为：环境尚无个人 Token，需要用户创建并写入本机私密 `.env`；取得 Token 后仍需采集认证后的 Target、业务组、数据源查询、历史范围、分页和错误响应脱敏夹具。在证据齐全前不实现猜测接口。
...
2. 用户创建 Nightingale 个人 Token 并写入本机私密 `.env` 后，继续采集认证只读 API 证据，再编写第二阶段独立规格和实施计划。

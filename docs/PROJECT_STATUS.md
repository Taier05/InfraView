...
主机列表已进一步压缩纵向占用，当前表格为主机名、IP 地址、CPU、内存、负载、IO 读/写、网络出/入、运行时间、状态共 9 列。每个字段保持单行显示，磁盘和网络速率自动换算为易读单位，过长内容使用省略号并可通过悬停查看完整值，使每台主机稳定只占一行。主机详情入口和前端路由暂时停用，详情实现文件保留，后续可按需要恢复。

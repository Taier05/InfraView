...
当前没有已知 Nightingale v8.4.1 API 契约或部署验收阻塞。开放事项是每个私有部署必须使用专用最小只读 Token；磁盘容量和磁盘读写历史缺少已确认指标契约，因此保持空序列。当前分支改动尚未推送或合并 `main`。
...
1. 完成最终分支差异、敏感信息和验证证据审查。
2. 等待用户确认后再决定推送或合并。
3. 后续落实专用最小只读 Token，并按证据补充磁盘指标。

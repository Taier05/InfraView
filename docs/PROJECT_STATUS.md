...
主机列表已进一步压缩纵向占用，当前表格为主机名、IP 地址、CPU、内存、负载、IO 忙碌度、网络出/入、运行时间、状态共 9 列。每个字段保持单行显示；网络列按“发送 / 接收”顺序只展示数值并自动换算单位。CPU、内存和 IO 忙碌度使用 80%/90% 警告与严重阈值，网络发送、接收分别使用可配置的 B/s 阈值着色。主机详情页、详情测试、专用图表组件和 ECharts 前端依赖已删除。

...
## 列表单页最多 500 条、共享运行时长与硬盘错误摘要（隔离 feature 分支本地交付）

- [x] 七个只读列表将固定 `page_size` 白名单扩展为 `20|50|100|500`，保持完整结果先服务端排序后分页、溢出保护和既有响应结构。
- [x] 共享分页控件新增普通“500 条”选项，七页 URL/请求/响应校验均支持 500；超过上限继续分页，不截断、不拼接页面。
- [x] 普通“500 条”文案完成聚焦测试、类型检查、production build 与镜像全量门禁，并已原位重建现有测试 8080；未合并、未推送。
...
- [x] 统一 `meta.collected_at` 为响应内最新有效 Nightingale 样本时间；stale 不改写时间，空快照不伪造时间。
- [x] 删除正常态刷新按钮及浏览器刷新时间/周期文案，保留 15 秒自动轮询与错误态重试。
- [x] 列表页展示模块最新数据时间；总览七张卡分别展示自己的数据时间。
- [x] Elasticsearch 固定 26 查询 batch 仅要求第 25/26 组 inventory 非 `null`，第 1–24 组单项缺失安全降级。
- [x] 完成 ES Provider、Service/HTTP 与前端页面定向 RED→GREEN。
- [x] 完成本轮最终全仓普通/race、前端 16 文件/273 项、typecheck/build、Playwright 5 文件/27 项静态发现、无缓存镜像及安全扫描。
- [x] 经用户授权原位重建现有测试 8080，并通过健康、唯一端口与容器安全基线验收；用户视觉验收确认结果良好。
- [x] 经用户授权创建功能提交 `ad6f01b`，快进合并到 `main` 并推送至 `origin/main`。
...
- [x] 在隔离工作树完成 Java 展示优化的容器门禁：16 个前端测试文件、275 项测试、typecheck、生产构建与 27 项 Playwright 静态清单均通过；静态扫描确认固定 GET、无写方法及无破坏性能力，未启动服务或浏览器。
- [x] 记录健康、端口、进程和端口进程一致性的三类二值状态展示：正常绿色“正常”、异常红色“异常”、缺失灰色“暂无数据”；五项业务端精确中文标签只影响显示，未知值、筛选控件、URL 与 API 均保持原始代码值兼容。

## 旧模块列表统一与 Java 展示收口

- [x] 主机、硬盘、MySQL、Redis 分别收口为 12、10、14、13 个紧凑单行列；仅保留硬盘错误摘要单列及 MySQL/Redis 当前连接/最大连接两个已确认例外。
- [x] 所有展示列接入固定 URL/HTTP/Service 排序白名单，在完整筛选结果上服务端排序；升降序缺失值始终末置，稳定实体 ID 收口，表头不渲染排序箭头。
- [x] 继续保留 Java 13 列、健康/端口/进程/端口进程一致性的三类等级色，以及五项业务端的中文显示和原始筛选代码契约。
...
## Elasticsearch inventory 稳定性与共享观测表排版

- [x] 七页列表统一接入 `observability-table` 共享排版，同时保持各模块的既有业务、排序、URL 与只读契约。
- [x] Elasticsearch cluster/node 历史 inventory 改以有效真实上报时间选择最新候选；非法单条局部跳过。
- [x] 锁定同一最新上报时间的地址冲突只清空地址，后续更晚候选才能恢复该字段。
- [x] 锁定固定一次 26 查询 batch、缓存和 2/5 freshness 不变；真实 Provider 失败仍按 stale/503 降级，不增加 retry。
- [x] 完成 Go 全量、Node 全量（17 文件/330 项）、typecheck、production build、Playwright 静态发现及固定查询/只读/敏感/whitespace 扫描；详情见 `docs/TESTING.md`。
- [x] 最终整包审查通过后，按用户单独授权用产品提交 `05656d5` 原位重建现有 8080；健康、唯一端口、容器安全基线和未认证 API 边界均通过，动态页面验收交由用户完成。
- [x] 用户视觉验收确认共享排版统一；补齐 Java 端口进程一致性三色徽标并再次原位部署。
- [x] 创建功能提交 `05656d5`，快进合并到 `main` 并推送至 `origin/main`；合并后的 Node 与 Go 全量门禁再次通过。
...
- [x] 【2026-08-01 当时状态】将硬盘页拆成型号、容量独立十列，容量可升降序；当时分页仍为 20/50/100，未增加命令超时、失联移除或“全部”。该历史分页状态已被本文件顶端的 500 条分页交付替代；命令超时仍未实施。

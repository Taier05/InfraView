...
- [x] 完成 MySQL 表格细化：新增 Buffer Pool 容量、稳定实例标签精确筛选、11 列语义收紧、状态列表头对齐和原 8080 Chromium 4/4；固定 14 条查询仍为一次 batch、无 N+1。
- [ ] 按真实契约证据再决定是否扩展 MySQL 指标；当前固定 14 条查询、一次 batch 和无 N+1 设计不接受前端自定义查询。
